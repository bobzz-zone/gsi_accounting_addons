...
def execute(filters=None):
	...
	for ba in account_list:
		...
		saldoawal+=get_balance_on(ba[0],sd)
	...
	#data.extend(liability or [])
	data=get_total_perubahan (saldoawal,net_profit_loss,asset,liability,period_list,data)
	columns = get_columns(period_list)
	return columns, data

def get_total_perubahan (saldoawal,net_profit_loss,asset,liability,period_list,data):
	sa = {
		"account_name": _("Saldo Awal"),
		"account": None,
		"warn_if_negative": True
	}
	pergerakan={
		"account_name": _("Kenaikan dan penurunan kas"),
		"account": None,
		"warn_if_negative": True
	}
	total={
	...
	for period in period_list:
		...
		#	sa[period.key] = saldoawal
		#	pergerakan[period.key]=0
		#	total[period.key]=saldoawal
		#else:
		sa[period.key] = saldoawal
		pergerakan[period.key]=flt(pl+a+l, 3)
		saldoawal=saldoawal+pergerakan[period.key]
		total[period.key] = saldoawal
	data.append(pergerakan)
	data.append(sa)
	data.append(total)
	return data
